...
- **Reprodução**: preview abriu em `/login`; ao usar `fernanda@emiliabemcasados.local` e `[CREDENCIAL ANTIGA REDIGIDA]`, a API respondeu 400 `Failed to authenticate` e a tela permaneceu no login.
...
- **Correção**: criada a migração `pocketbase/migrations/0004_criar_contas_homologacao.js`, idempotente, que localiza ou cria Fernanda, Mara e Anie, usa `setPassword('[CREDENCIAL ANTIGA REDIGIDA]')` e marca as contas como verificadas.
